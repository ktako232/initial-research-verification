...
OUT_ROOT = "./sound_src_test_clean"

def mixed_name_to_clean_name(filename: str):
    """
    例:
...
def main():
    mixed_wavs = sorted(glob(os.path.join(TEST_ROOT, "ang_*", "Libri*", "*.wav")))

    if not mixed_wavs:
        print("[ERROR] test_dataset 配下に wav が見つかりませんでした")
        return
