...
def main():
    mixed_wavs = sorted(glob(os.path.join(VAL_ROOT, "ang_*", "Libri*", "*.wav")))

    if not mixed_wavs:
        print("[ERROR] val_dataset 配下に wav が見つかりませんでした")
        return
    ...
    for mixed_path in mixed_wavs:
        rel_path = os.path.relpath(mixed_path, VAL_ROOT)
